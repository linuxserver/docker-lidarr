pipeline {
  ...
  environment {
    ...
    JSON_URL = 'https://services.lidarr.audio/v1/update/nightly/changes?os=linux'
    JSON_PATH = '.[0].version'
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:arm32v6-${META_TAG} ${IMAGE}:arm32v6-preview"
          ...
          sh "docker push ${IMAGE}:arm32v6-preview"
          sh "docker push ${IMAGE}:arm64v8-preview"
          sh "docker manifest push --purge ${IMAGE}:preview || :"
          sh "docker manifest create ${IMAGE}:preview ${IMAGE}:amd64-preview ${IMAGE}:arm32v6-preview ${IMAGE}:arm64v8-preview"
          sh "docker manifest annotate ${IMAGE}:preview ${IMAGE}:arm32v6-preview --os linux --arch arm"
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}' to preview",\
             "type": "commit",\
